...
// From http://www.cplusplus.com/reference/algorithm/shuffle/?kw=shuffle
...
{
    // TODO: Using the insertion sort algorithm, sort s according to
    //       to the ordering relationship passed in as the parameter
    //       comp. There is a version on the class website.
...
//        swap(s[0], s[1]);
    ...
    for (i = 1; i < length; i++) {
        Sensor v(s[i].id);
        v.avg = s[i].avg;
        std::copy(std::begin(s[i].readings), std::end(s[i].readings), std::begin(v.readings));
        
        j = i;
        while (j > 0 && comp(v, s[j-1])) {
            s[j].id = s[j-1].id;
            s[j].avg = s[j-1].avg;
            std::copy(std::begin(s[j-1].readings), std::end(s[j-1].readings), std::begin(s[j].readings));

            j--;
        }
        s[j].id = v.id;
        s[j].avg = v.avg;
        std::copy(std::begin(v.readings), std::end(v.readings), std::begin(s[j].readings));
    }
}
...
int main()
{
    ...
    // Didn't compile on XCode 13.1 "Use of undeclared identifier 'random_shuffle'"
    ...
    std::shuffle(ids.begin(), ids.end(), std::default_random_engine(seed));  // from <algorithm>
      ...
      // destruction time.
    {
     ...
     for (auto iter = auxSensors.begin(); iter != auxSensors.end(); iter++) {
         auxSensorPtrs.push_back(&*iter);
     }
      ...
      sensors.clear();
      for (auto iter = auxSensorPtrs.begin(); iter != auxSensorPtrs.end(); iter++) {
